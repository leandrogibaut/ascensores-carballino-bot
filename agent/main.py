...
def formatear_resumen_solicitud(datos_raw: str) -> str:
    ...
    return "\n".join(lineas) if lineas else datos_raw
...
async def webhook_handler(request: Request):
    ...
    try:
        ...
        for msg in mensajes:
            ...
            # Detectar si Olivia completó una solicitud y notificar al grupo interno
            tag_match = re.search(r'\[SOLICITUD_COMPLETA:(.+?)\]', respuesta, re.DOTALL)
            if tag_match:
                datos_raw = tag_match.group(1).strip()
                # Formatear resumen legible para el grupo
                resumen = formatear_resumen_solicitud(datos_raw)
                await notificar_grupo_solicitud(msg.telefono, resumen)
                # Limpiar el tag interno antes de enviar al cliente
